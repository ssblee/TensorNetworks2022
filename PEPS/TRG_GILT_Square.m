function [Nsq,Sv] = TRG_GILT_Square (T,rgstep,Nkeep,varargin)
% < Description >
%
% [Nsq,Sv] = TRG_GILT_Square (T,rgstep,Nkeep [, options])
%
...
%                3 |               | 1 
    ...
    % % % % TODO (start) % % % %
    [Us{1},Vs{1},Sv{itr}] = TRG_Square_split(Ts{1},Nkeep,[3 4],[2 3 1],[3 1 2]);
    [Us{2},Vs{2}]         = TRG_Square_split(Ts{2},Nkeep,[2 3],[2 3 1],[2 1 3]);
    [Us{3},Vs{3}]         = TRG_Square_split(Ts{3},Nkeep,[1 2],[2 3 1],[3 1 2]);
    [Us{4},Vs{4}]         = TRG_Square_split(Ts{4},Nkeep,[1 4],[1 3 2],[3 1 2]);

    % obtain the coarse-grained tensors
    Ts{1} = contPlaq(Vs{2},Vs{3},Vs{4},Vs{1});
    Ts{2} = contPlaq(Us{2},Us{3},Us{4},Us{1});
    Ts(3:4) = Ts(1:2);
    % % % % TODO (end) % % % %
...
T12 = contract(Ts{1},4,[1 3],Ts{2},4,[1 3]);
T43 = contract(Ts{4},4,[1 3],Ts{3},4,[1 3]);
res = contract(T12,4,(1:4),T43,4,(1:4));
Nsq = Nsq*exp(log(res)/(4*(2^rgstep)));
% % % % TODO (end) % % % %
...
% % % % TODO (start) % % % %
[U,S,V] = svdTr(T,4,idU,Nkeep,[]);
S2 = diag(sqrt(S));
U = contract(U,numel(idU)+1,numel(idU)+1,S2,2,1,pU);
V = contract(S2,2,2,V,5-numel(idU),1,pV);
% % % % TODO (end) % % % %
...
for itg = (1:N_GILT)
    ...
    trs = contract(Uenv,3,[2 1],eye(size(Uenv,1)),2,[1 2]); % trace: trs(n) = trace(Uenv(:,:,n))

    % Determine the elements of t^\prime_i, according to Eq. (31) of
    % Hauru2018
    Se_rat = (Senv./epsilon).^2; % (S_i / epsilon_i)^2
    trs = trs.*(Se_rat./(1 + Se_rat)); % obtain t^\prime_i

    % A new part of R^\prime, to be split and absorbed
    Rp = contract(conj(Uenv),3,3,trs,2,1);
    
    % Decompose "Rp" into the left and right parts via SVD
    [U2,S2,V2] = svdTr(Rp,2,1,[],Skeep_GILT);
    U2 = U2*diag(sqrt(S2));
    V2 = diag(sqrt(S2))*V2;
    ...
    Uenv = contract(U2,2,1,Uenv,3,1);
    Uenv = contract(Uenv,3,2,V2,2,2,[1 3 2]);

    USenv = contract(Uenv,3,3,diag(Senv),2,1);
    [Uenv,Senv] = svdTr(USenv,3,[1 2],[],[]);
...
for it2 = (1:numel(Ts))
    ids = mod(it2+[0 1],4)+1;
    Ts{it2} = contract(Ts{it2},4,ids,conj(Ts{it2}),4,ids);
end

Tenv = contract(Ts{1},4,[2 4],Ts{4},4,[2 4]);
Tenv = contract(Tenv,4,[3 4],Ts{3},4,[2 4]);
Tenv = contract(Tenv,4,[3 4],Ts{2},4,[2 4]);
Tenv = permute(Tenv,[1 3 2 4]);
sz = size(Tenv,1);

% sanity check
if ~all([size(Tenv),ones(1,4-ndims(Tenv))] == sz*ones(1,4))
    error('ERR: The environment tensor has a wrong size.');
end

% reshape the rank-4 Tenv into a matrix form, for the following
% eigen-decomposition
Tenv = reshape(Tenv,(sz^2)*[1 1]);
[Uenv,Senv] = eig((Tenv+Tenv')/2); % Hermitianize & diagonalize
[Senv,ids] = sort(diag(Senv),'descend');
Uenv = Uenv(:,ids);

% remove non-positive eigenvalues that are from numerical noises
oks = (Senv <= 0);
Uenv(:,oks) = [];
Senv(oks) = [];

Uenv = reshape(Uenv,[sz sz size(Uenv,2)]); % convert: rank-2 -> rank-3
Senv = sqrt(Senv); % convert: eigenvalues -> singular values
% % % % TODO (end) % % % %
