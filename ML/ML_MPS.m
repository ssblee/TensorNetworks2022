function [M,cfun,err,lid,cfun_test,err_test,lid_test] = ML_MPS (M,F,y,F_test,y_test,Nkeep,estep)
% < Description >
%
...
for its = (1:numel(estep))
    ...
    for itN = (N:-1:2)
        ...
        M{itN-1} = contract(M{itN-1},4,4,diag(S),2,1,[1 4 2 3]); % leg order: left-right-physical-label
        M{itN} = permute(M{itN},[1 3 2]); % leg order: left-right-physical
        
        % update Flr{itn+1} in accordance with the updated M{itn}
        T = reshape(F(:,:,itN),[size(F,1) 1 size(F,2)]).*Flr{itN+2};
        Flr{itN+1} = contract(T,3,[2 3],M{itN},3,[2 3]);

        % [Unpublished; devised by S.Lee] normalize feature vectors
        Flr{itN+1} = ML_MPS_normalize(Flr{itN+1});
        
        % update Flr_test{itn+1} in accordance with the updated M{itn}
        T = reshape(F_test(:,:,itN),[size(F_test,1) 1 size(F_test,2)]).*Flr_test{itN+2};
        Flr_test{itN+1} = contract(T,3,[2 3],M{itN},3,[2 3]);
    ...
    for itN = (2:N)
        ...
        M{itN-1} = permute(M{itN-1},[1 3 2]); % leg order: left-right-physical
        M{itN} = contract(diag(S),2,2,M{itN},4,1,[1 3 2 4]); % leg order: left-right-physical-label
        
        % update Flr{itn} in accordance with the updated M{itn-1}
        T = reshape(F(:,:,itN-1),[size(F,1) 1 size(F,2)]).*Flr{itN-1};
        Flr{itN} = contract(T,3,[2 3],M{itN-1},3,[1 3]);

        % [Unpublished; devised by S.Lee] normalize feature vectors
        Flr{itN} = ML_MPS_normalize(Flr{itN});

        % update Flr_test{itn} in accordance with the updated M{itn-1}
        T = reshape(F_test(:,:,itN-1),[size(F_test,1) 1 size(F_test,2)]).*Flr_test{itN-1};
        Flr_test{itN} = contract(T,3,[2 3],M{itN-1},3,[1 3]);
...
fx = sum(fx.*F1,2);
fx = reshape(fx,[size(fx,1) size(fx,3) size(fx,4) size(fx,5) size(fx,6)]); % remove the singleton dimension
fx = sum(fx.*F2,2);
fx = reshape(fx,[size(fx,1) size(fx,3) size(fx,4) size(fx,5)]); % remove the singleton dimension
fx = sum(fx.*F3,2);
fx = reshape(fx,[size(fx,1) size(fx,3) size(fx,4)]); % remove the singleton dimension
fx = sum(fx.*F4,2);
fx = reshape(fx,[size(fx,1) size(fx,3)]);

[~,maxid] = max(fx,[],2);
err = 1 - sum( y((1:size(fx,1)).'+(maxid-1)*size(fx,1)) )/size(fx,1); % sum up the elements of y at the "right" locations
...
if nargout > 2 %~isempty(estep)
    ...
    dB = dB.*F1;
    dB = dB.*reshape(F2,[size(F2,1) 1 size(F2,2)]);
    dB = dB.*reshape(F3,[size(F3,1) 1 1 size(F3,2)]);
    dB = dB.*reshape(F4,[size(F4,1) ones(1,3) size(F4,2)]);
