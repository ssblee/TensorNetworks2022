function [ocont,RhoV2s,iscvg] = DMFT_Hubbard (U,mu,T,D,ozin,RhoV2in,Lambda,N,Nkeep)
% < Description >
%
% [ocont,RhoV2s] = DMFT_Hubbard (U,mu,T,D,ozin,RhoV2in,Lambda,N,Nkeep)
%
...
oks = (ocont <= max(ozin)) & (ocont >= min(ozin));
RhoV2s(oks,1) = interp1(ozin,RhoV2in,ocont(oks),'linear');
iscvg = false;
