function [Lambda,Gamma] = ortho_Orus (Lambda,Gamma)
% < Description >
%
% [Lambda,Gamma] = ortho_Orus (Lambda,Gamma)
%
...
% Ket tensor to construct the transfer operator from the right
M = contract(Gamma,3,2,diag(Lambda),2,1,[1 3 2]);

% Find the right dominant eigenvector for the transfer operator
XR = ortho_Orus_vec(M,true);

% Ket tensor to construct the transfer operator from the left
M = contract(diag(Lambda),2,2,Gamma,3,1);

% Find the left dominant eigenvector for the transfer operator
XL = ortho_Orus_vec(M,false);

% do SVD in Fig. 2(ii) of Orus2008
[U,Lambda,V] = svdTr(XL'*diag(Lambda)*XR,2,1,[],1e-8);

% contraction in Fig. 2(ii) of Orus2008
Gamma = contract(V/XR,2,2,Gamma,3,1);
Gamma = contract(Gamma,3,2,(XL')\U,2,1,[1 3 2]);
