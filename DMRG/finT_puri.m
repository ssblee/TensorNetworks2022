function [taus,lnZs,PTS] = finT_puri (Hs,dt,tmax,Nkeep,Nsweep)
% < Description >
%
% [taus,lnZs,PTS] = finT_puri (Hs,dt,tmax,Nkeep,Nsweep)
%
...
for itN = (1:numel(Hs))
    ...
    Udt = permute(reshape(V*diag(exp((-dt*(1+(mod(itN,2)==0))/2)*D))*V', ...
        ds([1 3 2 4])),[1 3 2 4]);
...
U_MPO = cell(1,N);
for itN = (1:N)
    T = contract(Os{3,itN},3,2,Os{2,itN},3,1);
    T = contract(T,4,3,Os{1,itN},3,1,[1 4 2 3 5]);
    
    if mod(itN,2) == 1
        % merge the right legs of the tensors Os([1 3],itN)
        Aright = getIdentity(T,3,T,5);
        U_MPO{itN} = contract(T,5,[3 5],Aright,3,[1 2]);
    else
        % take the "Aright" generated from the last iteration, to merge the
        % left legs
        U_MPO{itN} = contract(T,5,[3 5],conj(Aright),3,[1 2],[1 2 4 3]);
    end
end
% % % % TODO (end) % % % %
...
for it1 = (1:Nstep)
    ...
    PTS = mtimes_MPO(U_MPO,PTS,Nkeep,Nsweep);
    ...
    Tr = 1;
    for itN = (1:N)
        Tr = contract(PTS{itN},4,3,Tr,2,1);
        Tr = contract(Tr,4,[1 2 4],conj(PTS{itN}),4,[1 2 3]);
    end
