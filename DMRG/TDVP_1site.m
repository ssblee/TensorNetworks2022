function [ts,M,Ovals,EE] = TDVP_1site (M,Hs,O,Nkeep,dt,tmax,varargin)
% < Description >
%
% [ts,M,Ovals,EE] = TDVP_1site (M,Hs,O,Nkeep,dt,tmax [,'Krylov',nKrylov] [,'tol',tol])
%
% One-site Time-dependent variational principle (TDVP) method for
...
% show message
disp('1-site TDVP : Real-time evolution with local measurements');
...
for itt = (1:Nstep)
    % left -> right
    for itN = (1:(N-1))
        % time evolution of rank-3 tensor M{itN}, via TDVP_1site_expHA
        Anew = TDVP_1site_expHA (Hlr{itN},Hs{itN},Hlr{itN+2},M{itN},dt/2,nKrylov,tol); % half time step dt/2
        
        % update M{itN} and generate Cold by using Anew, via SVD
        [M{itN},S2,V2] =  ...
            svdTr(Anew,3,[1 3],Nkeep,-1); % set Skeep as -1, not to truncate even zero singular values
        ...
        Cnew = TDVP_1site_expHC (Hlr{itN+1},Hlr{itN+2},Cold,-dt/2,nKrylov,tol);  % half time step -dt/2
    ...
    itN = N; % right end
    M{itN} = TDVP_1site_expHA (Hlr{itN},Hs{itN},Hlr{itN+2},M{itN},dt,nKrylov,tol); % full time step dt at the right end

    % right -> left
    for itN = ((N-1):-1:1)
        % update M{itN+1} and generate Cold via SVD
        [U2,S2,M{itN+1}] = ...
            svdTr(M{itN+1},3,1,Nkeep,-1); % set Skeep as -1, not to truncate even zero singular values
        ...
        M{itN} = contract(M{itN},3,2,Cnew,2,1,[1 3 2]);
        
        % time evolution of site-canonical tensor ("A tensor") M{itN}, via TDVP_1site_expHA
        M{itN} = TDVP_1site_expHA (Hlr{itN},Hs{itN},Hlr{itN+2},M{itN},dt/2,nKrylov,tol); % half time step dt/2
    end
...
% The collection of rank-2 tensors as Krylov basis; the 3rd dimension is
% for indexing different rank-2 tensors
As = zeros([size(Aold,1),size(Aold,2),nKrylov]);

% Define the first Krylov vector as the input "Aold"
As(:,:,1) = Aold/sqrt(abs(contract(conj(Aold),2,(1:2),Aold,2,(1:2))));
% normalize; insert "abs" to avoid numerical noise
...
for itn = (1:nKrylov)
    % "matrix-vector" multiplication
    Amul = contract(Hleft,3,2,As(:,:,itn),2,1);
    Amul = contract(Amul,3,[2 3],Hright,3,[3 2]);

    alphas(itn) = real(contract(conj(As(:,:,itn)),2,(1:2),Amul,2,(1:2)));
    % insert "real" to avoid numerical noise
    ...
    if itn < nKrylov
        % orthogonalize, to get the next Krylov vector
        for it2 = (1:2) % do twice to further reduce numerical noise
            T = contract(conj(As(:,:,1:itn)),3,(1:2),Amul,2,(1:2));
            T = contract(As(:,:,1:itn),3,3,T,2,1);
            Amul = Amul - T;
        end
    
        % norm
        Anorm = sqrt(abs(contract(conj(Amul),2,(1:2),Amul,2,(1:2)))); % insert "abs" to avoid numerical noise
...
Anew = contract(As(:,:,1:cnt),3,3,Vkr*(diag(exp((-1i*dt)*diag(Ekr)))*Vkr(1,:)'),2,1);
